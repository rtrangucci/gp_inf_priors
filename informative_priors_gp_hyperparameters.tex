...
\begin{abstract}
  Gaussian process priors are a broad class of priors over functions, and as
  such, are a rich set of priors over conditional mean functions in Bayesian
  nonparameteric regression models. When faced with data, the analyst must make
  a choice as to how to infer the hyperparameters from the given dataset.
  However, literature on Gaussian processes elides this inferential step,
  typically settling on using the dataset once to use point-estimation
  techniques for the hyperparameters, and then using the dataset again to infer
  the resulting posterior conditional mean functions. Given that our
  probability of correctly picking our hyperparameters with finite data is
  exactly zero, we prefer to take a Bayesian approach to hyperparameter
  inference by placing priors over our hyperparameters and integrating over
  uncertainty.  This comes with its own problems, as kernel hyperparameters are
  weakly identified. This can lead to thick tails, which can hinder our ability
  to generate a proper Bayesian estimator under the MC-CLT. We formalize the
  characteristics of priors that lead to well-behaved posteriors over
  hyperparameters and find that weakly informative priors on length-scale
  parameters that limit probability mass at very large lenth-scales and very
  small length-scales are empirically sufficient for generating well-behaved
  MCMC chains in Stan. This follows logically from the limiting behavior of
  Gaussian processes as length-scale tends to large (small) values, and from
  the conditions under which probability densities admit geometrically ergodic
...
$\mathbf{y} \in \R, \mathbf{X} \in \R^M$, is:

\begin{align*}
  \alpha & \sim p(\mathbf{\alpha} | \theta_\mathbf{\alpha}) \\
  l & \sim g(\mathbf{l} | \theta_l) \\
  f(\mathbf{x}) & \sim GP\lp \mathbf{f}(\mathbf{x}) |\, \mu(\mathbf{x}), K(\mathbf{x}, \mathbf{x} | \mathbf{\alpha},\, l) \rp \\
\end{align*}

Data is then generated according to:

\begin{align*}
  X_i & \sim p(\mathbf{X} | \phi) \\
...
and its covariance matrix $K(\mathbf{\alpha}, \, l)$. With $\mathbf{\alpha}$ and $l$ fixed,
inference of $f(\mathbf{x} | \, \mathbf{Y}, \mathbf{\alpha}, l)$ is straightforward:
...
\begin{align*}
	...
	\end{pmatrix} ,\,\,
	\begin{pmatrix}
    K(\mathbf{x},\mathbf{x} | \mathbf{\alpha}, l) & K(\mathbf{x},\mathbf{x} | \mathbf{\alpha}, l)  \\
    K(\mathbf{x},\mathbf{x} | \mathbf{\alpha}, l) &  K(\mathbf{x},\mathbf{x} | \mathbf{\alpha}, l) + \sigma ^ 2 \mathbf{I}\\
	\end{pmatrix}
	\rp
\end{align*}


While the software machinery to fit GPs are abundant, there is a dearth of
...
function can be multimodal, and also have areas where the objection function
...
We have practical considerations too. Focusing on point estimates of our
hyperparameters 

As the gaussian process prior with either of these kernels admits nonlinear
functions, it is important to be able to control the nonlinearity in the
functions admitted by the prior. More importantly, 
R\&W prove that 
