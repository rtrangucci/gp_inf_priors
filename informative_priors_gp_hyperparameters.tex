...
\begin{abstract}
  ...
  nonparameteric regression models. Estimating hyperparameters in kernel
  functions is a thorny problem because these parameters are weakly identified
  by data. We derive a principled procedure to specify priors for
  hyperparameters in exponentiated quadratic kernels that places soft
  constraints the space of functions explored by the Gaussian process
  regression model. Our procedure follows logically from the limiting behavior of
...
$\mathbf{y} \in \R, \mathbf{X} \in \R^M$, is (adopting the notation of Flaxman et. al):

\begin{align*}
  \theta & \sim p(\Theta | \theta_\mathbf{\alpha}) \\ 
  f(\mathbf{x}) & \sim GP\lp \mathbf{f}(\mathbf{x})
  |\, \mu(\mathbf{x}), K_\theta(\mathbf{x}, \mathbf{x}) \rp \\
\end{align*}
...
Choosing a point estimates of our
hyperparameters leads to one sample function, 

More
importantly, R\&W prove that 
